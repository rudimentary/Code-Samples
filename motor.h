 ...
 * motor.h
 *
 * Header file for operation of L6470 motor controller on TI Stellaris LM4F120H5QR
 *  Created on: Apr 17, 2013
 *      Author: Eric Boe, Mateus Daczko
 *
...
//----------------------------------------------------------------------------------
// DEFINITIONS FROM dSPIN_example_v2.ino
//----------------------------------------------------------------------------------
...
#define dSPIN_STATUS_MOT_STATUS_STOPPED        0x0000 // Motor stopped
#define dSPIN_STATUS_MOT_STATUS_ACCELERATION   0x0020 // Motor accelerating
#define dSPIN_STATUS_MOT_STATUS_DECELERATION   0x0040 // Motor decelerating
#define dSPIN_STATUS_MOT_STATUS_CONST_SPD      0x0060 // Motor at constant speed
...
//-------------------------------------------------------------------------
// END DEFINITIONS FROM dSPIN_example_v2.ino
//-------------------------------------------------------------------------

#define dSPIN_INCH 				2000  // 10 revolutions per inch
#define dSPIN_REVO 				200   // 200 steps per revolution
#define dSPIN_STEL_MS			16667 // one millisecond of 3 * 20 ns cycles
#define dSPIN_TIMEOUT_SEC		60	  // one minute


//-------------------------------------------------------------------------
// FUNCTIONS FROM dSPIN_commands.ino - Contains high-level command implementations
//-------------------------------------------------------------------------
// For Motor A:
void dSPINA_SetParam(unsigned char, unsigned long);
unsigned long dSPINA_GetParam(unsigned char);
unsigned long dSPINA_ParamHandler(unsigned char, unsigned long);
void SetLSPDOptA(unsigned char);
void dSPINA_Run(unsigned char, unsigned long);
void dSPINA_Step_Clock(unsigned char);
void dSPINA_Move(unsigned char, unsigned long);
void dSPINA_GoTo(unsigned long);
void dSPINA_GoTo_DIR(unsigned char, unsigned long);
void dSPINA_GoUntil(unsigned char, unsigned char, unsigned long);
void dSPINA_ReleaseSW(unsigned char, unsigned char);
void dSPINA_GoHome();
void dSPINA_GoMark();
...
// For Motor B:
void dSPINB_SetParam(unsigned char, unsigned long);
unsigned long dSPINB_GetParam(unsigned char);
unsigned long dSPINB_ParamHandler(unsigned char, unsigned long);
void SetLSPDOptB(unsigned char);
void dSPINB_Run(unsigned char, unsigned long);
void dSPINB_Step_Clock(unsigned char);
void dSPINB_Move(unsigned char, unsigned long);
void dSPINB_GoTo(unsigned long);
void dSPINB_GoTo_DIR(unsigned char, unsigned long);
void dSPINB_GoUntil(unsigned char, unsigned char, unsigned long);
void dSPINB_ReleaseSW(unsigned char, unsigned char);
void dSPINB_GoHome();
void dSPINB_GoMark();
...
//-------------------------------------------------------------------------
// END FUNCTIONS FROM dSPIN_commands.ino
//-------------------------------------------------------------------------

//-------------------------------------------------------------------------
// FUNCTIONS FROM dSPIN_support.ino - Contains functions used to implement the high-level commands
//-------------------------------------------------------------------------
...
unsigned long dSPINA_Param(unsigned long, unsigned char);
unsigned char dSPINA_Xfer(unsigned char);
unsigned long dSPINB_Param(unsigned long, unsigned char);
unsigned char dSPINB_Xfer(unsigned char);

//-------------------------------------------------------------------------
// END FUNCTIONS FROM dSPIN_support.ino
//-------------------------------------------------------------------------

// Control Functions used internally
void dSPINinit(void);

void dSPINreset(void);


int dSPIN_Ready(void);

//-------------------------------------------------------------------------
// CODE FOR USING MOVE SOLUTION FROM PUZZLE
//-------------------------------------------------------------------------

int MoveTiles(/*Proprietary Magic*/);
